#!/usr/bin/env bash
# Strict per-platform release build. CI combines the two independently built
# platform parts and only then creates the manifest that is signed offline.
set -euo pipefail
...
NATIVE="heatdeath-${PLATFORM}-${ARCH}"
...
if [ "$(node -v)" != "$NODE_EXPECTED" ]; then
  echo "FATAL: SEA release requires Node $NODE_EXPECTED" >&2
  exit 1
fi
case "$PLATFORM/$ARCH" in
  darwin/arm64|linux/x64) ;;
  *) echo "FATAL: unsupported release platform $PLATFORM/$ARCH" >&2; exit 1 ;;
esac
...
mv dist/heatdeath "dist/$NATIVE"
chmod 0755 "dist/$NATIVE"

guard_output="$("./dist/$NATIVE" --prove-guard 2>&1)"
...
native_hash="$(shasum -a 256 "dist/$NATIVE" | awk '{print $1}')"
...
node build/release-provenance.mjs \
  "$commit" "$node_hash" "$NATIVE" "$native_hash" \
  "$SOURCE_ARCHIVE" "$source_hash" \
  "$SBOM" "$sbom_hash" "$NPM_EXPECTED"

echo "$PLATFORM/$ARCH release part built; CI must combine both platform parts"
